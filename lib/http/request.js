var _ = require('lodash')
  ...
  , log = require('../logger').getLogger('dodo.core.http.request');
...
function PromisifiedRequest (method, url) {
  log.warning("dodo/lib/http/request will be deprecated it is replaced with superagent + some plugins (to promisify it) in dodo/lib/utils/request.");
  this.method_ = method;
  this.url_ = null;
  this.timeout_ = null;
  this.headers_ = {};
...
PromisifiedRequest.prototype.clone = function () {
  if (this.toStream_ || this.fromStream_ || !_.every(this.formFiles_, _.isString)) {
...
PromisifiedRequest.prototype.configureRequest_ = function (req) {
  ...
  if (self.timeout_) {
    req.on('socket', function (socket) {
      socket.setTimeout(self.timeout_);
